    ...
    register: async (req, res) => {
        try {
            ...
            if (checkEmail) {
              ...
              const customerAcc =  await customerModel.create({ userName, email, phoneNumber, address, hashPassword })

                //Khi tạo 1 customer mới sẽ tạo luôn 1 giỏ hàng cho customer đấy
                await cartModel.create({idCustomer:customerAcc._id})
                return res.status(200)
                    .json({ success: true, message: "Register success",data:customerAcc })
            }
        } catch (error) {
            res.send(error)
        }
